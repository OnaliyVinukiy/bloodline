...
> = ({ onPreviousStep, selectedDate, startTime, endTime }) => {
  ...
  const [province, setProvince] = useState<
    { province_id: string; province_name_si: string }[]
  >([]);

  const [district, setDistrict] = useState<
    { district_id: string; district_name_si: string; province_id: string }[]
  >([]);

  const [city, setCity] = useState<
    { city_id: string; city_name_si: string; district_id: string }[]
  >([]);
  ...
  const fetchDistricts = async (provinceName: string) => {
    try {
      const response = await fetch(
        `${backendURL}/api/districts/province-name/${provinceName}?lang=si`
      );
  ...
  const fetchCities = async (districtName: string) => {
    try {
      const response = await fetch(
        `${backendURL}/api/city/district/${districtName}?lang=si`
      );
  ...
  const validateForm = () => {
    ...
    if (!formData.fullName.trim()) newErrors.fullName = "සම්පූර්ණ නම අවශ්‍යයි.";
    if (!formData.nic.trim())
      newErrors.nic = "ජාතික හැඳුනුම්පත් අංකය අවශ්‍යයි.";

    if (!formData.contactNumber.trim())
      newErrors.contactNumber = "දුරකථන අංකය අවශ්‍යයි.";
    if (!formData.province.trim()) newErrors.province = "පළාත අවශ්‍යයි.";
    if (!formData.district.trim())
      newErrors.district = "දිස්ත්‍රික්කය අවශ්‍යයි.";
    if (!formData.city.trim()) newErrors.city = "නගරය අවශ්‍යයි.";
    if (!formData.googleMapLink.trim())
      newErrors.googleMapLink = "ගූගල් සිතියම් සබැඳිය අවශ්‍යයි.";
      ...
      <main className="mt-0 mb-8 w-full max-w-4xl px-4 py-8">
        <div className="relative bg-white rounded-xl shadow-2xl overflow-hidden p-1">
          ...
          <div className="relative bg-white rounded-xl p-8 z-10">
            ...
            {/* Form */}
            <div className="bg-gray-50 p-8 rounded-lg border border-gray-100 font-notoserif shadow-sm">
              <Label
                htmlFor="province"
                className="block mb-6 text-lg font-notoserif font-medium text-gray-800"
              >
                සංවිධායකයාගේ විස්තර
              </Label>
              <div className="mt-4 space-y-6">
                ...
                <div className="w-full">
                  ...
                  >
                    {" "}
                    සංවිධායකයාගේ සම්පූර්ණ නම (ජාතික හැඳුනුම්පතේ ඇති පරිදි)*
                  </Label>
                  <input
                    type="text"
                    name="fullName"
                ...
                <div className="w-full">
                  ...
                  <input
                    ...
                    value={formData.nic}
                    onChange={handleInputChange}
                    placeholder="ජාතික හැඳුනුම්පත් අංකය ඇතුලත් කරන්න"
                    className="bg-indigo-50 border border-indigo-300 text-indigo-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                    required
                  />
                  {errors.nic && (
                ...
                <div className="w-full">
                  ...
                  <input
                    ...
                    disabled
                    placeholder="ඊමේල් ලිපිනය ඇතුලත් කරන්න"
                    className="bg-indigo-50 border border-indigo-300 text-indigo-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                    required
                  />
                </div>
                <div className="w-full">
                  ...
                  {!isPhoneNumberValid && (
                    <p className="text-sm text-red-500 mt-1">
                      කරුණාකර 0 සිට ආරම්භ වන ඉලක්කම් 10 ක වලංගු දුරකථන අංකයක්
                      ඇතුළත් කරන්න.
                    </p>
                  )}
                </div>
                ...
                <Label
                  htmlFor="province"
                  className="mt-2 block mb-2 text-lg font-medium font-notoserif text-gray-800 mt-2"
                >
                  ස්ථානය පිළිබඳ විස්තර
                </Label>
                ...
                <div className="flex flex-col md:flex-row md:space-x-6 space-y-4 md:space-y-0 mb-6">
                  <div className="w-full md:w-1/3">
                    ...
                    >
                      <option value="">පළාතක් තෝරන්න</option>
                      {province.map((prov) => (
                        <option
                          key={prov.province_id}
                          value={prov.province_name_si}
                        >
                      ...
                      {district.length > 0 ? (
                        district.map((districtItem) => (
                          <option
                            ...
                            value={districtItem.district_name_si}
                          >
                            {districtItem.district_name_si}
                          </option>
                        ))
                      ) : (
                        <option value="">කරුණාකර පළමුව පළාතක් තෝරන්න.</option>
                      )}
                    </select>
                    {errors.district && (
                      <p className="text-red-500 text-xs mt-1">
                  ...
                  <div className="w-full md:w-1/3">
                    ...
                    >
                      ...
                      {city.length > 0 ? (
                        ...
                        <option value="">
                          කරුණාකර පළමුව දිස්ත්‍රික්කයක් තෝරන්න.
                        </option>
                      )}
                    </select>
                    {errors.city && (
              ...
              <div className="mt-6 space-y-6">
                <div className="w-full">
                  <Label
                    ...
                    className="block mb-2 text-sm font-medium text-indigo-900"
                  >
                    ස්ථානය ඇතුලත් කර සිතියමෙන් තෝරන්න*
                  </Label>
                  <input
                    ref={searchInputRef}
                    type="text"
                    id="searchLocation"
                    placeholder="ස්ථානය සොයන්න"
                    className="bg-indigo-50 border border-indigo-300 text-indigo-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                  ...
                  <div className="mt-2 text-sm text-gray-500">
                    <span>ස්ථානය සොයා සිතියම මත එය තෝරන්න. </span>
                  </div>
                </div>
                <div className="w-full">
                  <Label
                    ...
                    className="block mb-2 text-sm font-medium text-indigo-900"
                  >
                    ස්ථානය සඳහා ගූගල් සිතියම් සබැඳිය*
                  </Label>
                  <input
                    type="text"
                    name="googleMapLink"
                    value={formData.googleMapLink}
                    onChange={handleInputChange}
                    placeholder="සබැඳිය ඇතුළු කරන්න"
                    disabled
                    className="bg-indigo-50 border border-indigo-300 text-indigo-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                    required
                  />
          ...
          <Modal show={showModal} onClose={() => handleCloseModal()}>
            ...
            <Modal.Body>
              <p className="text-lg text-gray-700">
                රුධිර පරිත්‍යාග කඳවුරක් සඳහා වන ඔබගේ ඉල්ලීම සාර්ථකව ඉදිරිපත් කර
                ඇත. අපගේ කණ්ඩායම ඔබගේ ඉල්ලීම සමාලෝචනය කරනු ඇති අතර, එය අනුමත වූ
                හෝ ප්‍රතික්ෂේප වූ පසු ඔබට තහවුරු කිරීමේ ඊමේල් පණිවිඩයක් ලැබෙනු
                ඇත. ජීවිත බේරා ගැනීමට ඔබ දක්වන සහයෝගයට ස්තූතියි!
              </p>
            </Modal.Body>
          </Modal>
        </div>
