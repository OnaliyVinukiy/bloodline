...
const RejectedCamps = ({ camps }: { camps: Camp[] }) => {
  ...
  // Filter rejected camps
  const rejectedCamps = camps.filter((camp) => camp.status === "Rejected");

  // Initialize filtered camps
  useEffect(() => {
    setFilteredCamps(rejectedCamps);
  }, [camps]);
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [rejectedCamps, searchTerm, dateFilter, cityFilter, filterType]);
